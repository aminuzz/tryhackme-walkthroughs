...
- Nothing of interest here, just the landing page for the website but it told me that this website was using `php` for their backend.
...
- It is worth noting that the `panel` page was accessible without any authentication which itself is an example of **broken access control** since an anonymous user is able to access an admin style upload page
...
nc -lnvp 1234
```
- `l` listen, `-v` verbose, `-n` skip DNS resolution, `-p` port. The port has to match the one reflected in the script
...
- Apache is commonly configured to execute multiple PHP related extensions as PHP code however the upload filter was only checking files that contain a `.php` extension specifically. `.php5` bypasses this filter completely while Apache hands the file to the PHP interpreter for execution. Whenever you hit a extension blocklist look up alternate extensions the server's runtime may use to execute
...
The final step was to find a way to elevate our privileges. In most cases, I usually check what the user can run with `sudo` by running `sudo -l` however we gained access by using a reverse shell and we do not know the user's `sudo` password so that wouldn't work.
...
- In this case, Python with the **SUID** bit set lets you spawn a subshell which inherits `root` privileges. 
...
- Using tools like `linpeas` or `LinEnum` to automate a deep privilege escalation path finder script would've been the ideal way to go about this, however for this challenge finding the SUID was enough but on a harder target I'd run them to make sure I wasn't missing any easier paths.
